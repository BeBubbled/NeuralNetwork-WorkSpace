...
def read_squad(path):
    ...
    # id_map里存的是question_hash_id->[answers'_index]
    # 需要注意的是同一组question, context, answer, id对应的index都是一样的
    id_map = ddict(list)
    for idx, qid in enumerate(data_dict['id']):
        id_map[qid].append(idx)

    # 这一步就是为了压缩空间，将1*context:1*question:1*answer的存储形式改成了1*context:1*question:n*answer的形式
    data_dict_collapsed = {'question': [], 'context': [], 'id': []}
    if data_dict['answer']:
        data_dict_collapsed['answer'] = []
    # 一个qid对应一个question
    for qid in id_map:
        # ex_ids是这个question的所有answer的index
        ex_ids = id_map[qid]
        ...
        # 和上面两行一样的，也可以写成：
        # data_dict_collapsed['id'].append(data_dict['id'][ex_ids[0]])
